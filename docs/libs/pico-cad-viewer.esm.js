...
function loadModel(gl, model, tn) {
  ...
  for (const object of model.objects) {
    ...
    for (const face of object.faces) {
      ...
      const pass = passes[(face.doubleSided ? 0 : 1) + (face.shading ? 0 : 2) + (face.texture ? 0 : 4) + (face.renderFirst ? 0 : 8)]
      ...
      if (faceIndices.length === 4 && face.texture && tn > 1) {
        ...
        for (let xi = 0; xi <= tn; xi++) {
          ...
          const p0 = [lerp(c0[0], c1[0], xt), lerp(c0[1], c1[1], xt), lerp(c0[2], c1[2], xt), lerp(uv0[0], uv1[0], xt), lerp(uv0[1], uv1[1], xt)]
          const p1 = [lerp(c3[0], c2[0], xt), lerp(c3[1], c2[1], xt), lerp(c3[2], c2[2], xt), lerp(uv3[0], uv2[0], xt), lerp(uv3[1], uv2[1], xt)]
...
class PicoCADViewer {
  ...
  setResolution(width, height, scale = 1) {
    ...
    if (isSafari) {
      ...
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, this._frameBufferTex, 0)
  ...
  _createTexture(tex, internalFormat, format, type, source, width, height) {
    ...
    if (width == null) {
      gl.texImage2D(gl.TEXTURE_2D, 0, internalFormat, format, type, /** @type {TexImageSource} */ (source))
    } else {
      gl.texImage2D(gl.TEXTURE_2D, 0, internalFormat, width, height, 0, format, type, /** @type {ArrayBufferView} */ (source))
    }
  ...
  draw() {
    ...
    // Setup camera
    const mat = create$1()
    perspective(mat, (this.cameraFOV * Math.PI) / 180, this._resolution[0] / this._resolution[1], 0.1, 400)
    ...
    if (this.drawWireframe) {
      ...
      gl.uniform4fv(this._programWireframe.locations.color, [this.wireframeColor[0], this.wireframeColor[1], this.wireframeColor[2], 1])

      // Bind vertex data
      gl.bindBuffer(gl.ARRAY_BUFFER, this._wireframe.vertexBuffer)
      gl.vertexAttribPointer(this._programWireframe.program.vertexLocation, 3, gl.FLOAT, false, 0, 0)
    ...
    if (isSafari) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, this._screenQuads)
      gl.vertexAttribPointer(this._programFramebuffer.program.vertexLocation, 2, gl.FLOAT, false, 0, 0)
